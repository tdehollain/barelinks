...
export function LinksTable() {
  ...
  const { page, term: searchParamTerm, tag: searchParamTag } = useSearch({ from: '/' });
  ...
  const queryResult = useQuery(
    ...
    isLoaded && isSignedIn
      ? {
          ...
          term: (searchParamTerm ?? '').length > 0 ? searchParamTerm : undefined,
        }
      : 'skip',
  );
  const tagsResult = useQuery(api.tags.getTagsWithUsage, isLoaded && isSignedIn ? {} : 'skip');
  ...
  if (!isLoaded) {
    return <div className="w-full max-w-5xl mx-auto mt-8 text-center text-muted-foreground">Loading…</div>;
  }

  if (!isSignedIn) {
    return <div className="w-full max-w-5xl mx-auto mt-8 text-center text-muted-foreground">Please sign in to view your links.</div>;
  }
  ...
  const handleDetachTag = async (linkId: Id<'links'>, tagId: Id<'tags'>) => {
    ...
    try {
      ...
      setRemovingTagKey((currentKey) => (currentKey === key ? null : currentKey));
    }
  };
  ...
  const totalPages =
    typeof totalLinks === 'number' ? Math.max(1, Math.ceil(totalLinks / LINKS_PER_PAGE)) : currentPageIndex + 1 + (hasMore ? 1 : 0);
  const currentPageDisplay = currentPageIndex + 1;
  const trimmedSearchTerm = (searchParamTerm ?? '').trim();
  const activeTag = selectedTagId ? tagsWithUsage.find(({ tag }) => tag._id === selectedTagId)?.tag ?? null : null;

  const termForSearchParam = trimmedSearchTerm.length > 0 ? trimmedSearchTerm : undefined;
  ...
  return (
    <div className="w-full max-w-5xl mx-auto space-y-3 mb-8">
      ...
      {isLoadingLinks ? (
        <div className="w-full mt-4 text-center text-muted-foreground">Loading your links…</div>
      ) : links.length === 0 ? (
        <div className="w-full mt-4 text-center text-muted-foreground">{noResultsMessage}</div>
      ) : (
        links.map((link) => (
