...
export function LinksTable() {
  ...
  const { data, isLoading, error, isFetching, isPlaceholderData } = useQuery({
    ...
    queryFn: () =>
      fetchLinks(currentPage, pageSize, searchKeyword, selectedTagIds),
  ...
  const deleteMutation = useMutation({
    ...
    onMutate: async (deletedLinkId) => {
      ...
      queryClient.setQueryData(
        ['links', currentPage, pageSize, searchKeyword, selectedTagIds],
        (old: LinksResponse | undefined) => {
          if (!old) return old;
          return {
            ...old,
      ...
      {isLoading ? (
        ...
        <>
          <div className="w-full max-w-5xl mx-auto mt-8">
            ...
            <div
              className={`space-y-1 ${isPlaceholderData ? 'opacity-60' : ''}`}
            >
              {links.map((link: Link) => (
                <div
                  key={link.id}
